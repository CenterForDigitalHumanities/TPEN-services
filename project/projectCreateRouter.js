...
router.route("/create").post(auth0Middleware(), screenContentMiddleware(), async (req, res) => {
  ...
  let sus = false
  // Some complex structures of the project data in the request body were not checked by screenContentMiddleware
  try {
    // Check those complex structures
    sus = hasSuspiciousProjectData(project)
    if (sus) return respondWithError(res, 400, "Suspicious project data will not be processed.")
  }
  catch (err) {
    // This only happens in project.metadata. project.tools, project.layers, or project.layers[n].pages was malformed.
    // It will 400 downstream?
    // return respondWithError(res, 400, "Invalid project data")
  }
  try {
