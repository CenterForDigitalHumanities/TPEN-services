...
function validateQueries({
  ...
  if (!(typeof hasRoles === "string" || Array.isArray(hasRoles)))
    ...
    validation = [
      false,
      'exceptRoles must be either "NONE" or a list of roles.'
    ]
  else if (
    createdBefore &&
    !(parseInt(createdBefore) !== NaN || createdBefore === "NOW")
  )
    validation = [
      false,
      'createdBefore must be either "NOW" or a date in UNIX time.'
    ]
  else if (
    modifiedBefore &&
    !(parseInt(modifiedBefore) !== NaN || modifiedBefore === "NOW")
  )
    validation = [
      false,
      'modifiedBefore must be either "NOW" or a date in UNIX time.'
    ]
    ...
    hasCollaborators &&
    !(hasCollaborators === "true" || hasCollaborators === "false")
  )
    validation = [
      false,
      "hasCollaborators must be a boolean or left undefined."
    ]
  else if (tags && !Array.isArray(tags))
    validation = [
      false,
      "tags must be either an array of strings or left undefined."
    ]
...
export async function respondWithProjects(user, options, res) {
  ...
  projects = projects.filter(
    (project) =>
      createdAfter < project.created && project.created < createdBefore
  )
...
router
  ...
  .get(auth0Middleware(), (req, res, next) => {
    respondWithProjects(
      req.user?.["http://store.rerum.io/agent"] ?? 404,
      req.query,
      res
    )
  })
